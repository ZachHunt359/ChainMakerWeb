...
export default function Index() {
  ...
  return (
    <div className="space-evenly">
      ...
      <div></div>
      <div className="center-text-align vcentered">
        <div className="rounded-rect error-highlight extra-roomy-cell bold center-text-align clickable neutral-highlight-h mild-outline-h"
          onClick={() => {
            if (!confirm("Are you sure you want to delete the entire chain? This cannot be undone.")) return;
            if (!confirm("Last chance: you are about to permanently delete the entire chain.")) return;
            if (localStorage.getItem("recentChains"))
              localStorage.setItem("recentChains", JSON.stringify(
                (JSON.parse(localStorage.getItem("recentChains")!) as { name: string, id: string }[]).filter(({ id }) => String(id) != chainId)
              ))
            submit({ id: chainId }, {
              action: "/api/delete",
              method: "POST",
